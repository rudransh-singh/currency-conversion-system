...
PtrToGraphList CreateGraph(int NumberOfVertices)
{
    ...
    G->GraphVertexArray=(PtrToGraphNode*)malloc(NumberOfVertices*sizeof(GraphNode));
...
void InsertEdge(PtrToGraphList G,int source,int destination,int inputweight)//this function will add an edge to the graphlist from the source vertex to the destination 
{
 ...
 for(;temp->next;temp=temp->next)//traversing through the list of the source vertex to check whether the edge already exists
 {
...
void PrintGraph(PtrToGraphList G)
{
    ...
    {   
        ...
        for(temp=G->GraphVertexArray[i];temp->next->next;temp=temp->next)
        {
